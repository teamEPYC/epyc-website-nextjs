...
async function main() {
  ...
  try {
    // Order matters on the down: Projects must be wiped before Blogs because
    // Blogs' down() deletes the shared `media` collection (which Project
    // thumbnails reference). On the up, Blogs goes first by convention; either
    // order works there since the two seeds write to disjoint collections.
